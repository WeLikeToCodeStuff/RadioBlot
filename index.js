...
for (var i = 0; i < 4; i++) {
  ...
  square(22.25+(i * 5),46,5,4,buttons)
  buttons
    .jump([24.8+(i * 5),43])
    .arc(360, 1)
...
const freq = `${Math.floor(bt.rand() * 10)}${Math.floor(bt.rand() * 10)}${Math.floor(bt.rand() * 10)}.${Math.floor(bt.rand() * 10)}${Math.floor(bt.rand() * 10)}${Math.floor(bt.rand() * 10)} MHz`
DrawText(freq, [27,55],0.5)
